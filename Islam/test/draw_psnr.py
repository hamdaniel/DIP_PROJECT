...
"""
lstm_ssim = np.genfromtxt("Islam/test/jpeg_psnr.csv", delimiter=",")
...
"""
"""
bpg_psnr = np.genfromtxt("Islam/test/lstm_psnr.csv", delimiter=",")
bpg_psnr = bpg_psnr[:, :-1]
...
gdn_psnr = np.genfromtxt("Islam/test/gdn_psnr.csv", delimiter=",")
gdn_psnr = gdn_psnr[:, :-1]
gdn_bpp = np.arange(1, gdn_psnr.shape[-1]+1) / 192 * 24
print(gdn_psnr.shape, gdn_bpp.shape)
gdn_psnr = gdn_psnr.mean(axis=0)
plt.plot(gdn_bpp, gdn_psnr, marker='o')
'''
webP_psnr = np.genfromtxt("Islam/test/webP_psnr.csv", delimiter=",")
